...
# 随机窗口采样
def generate_train_dataset(image_num = 50,
    ...
    images_name = os.listdir("./dataset/origin/images_png")
    images_path = glob.glob("./dataset/origin/images_png/*")
    labels_path = glob.glob("./dataset/origin/annotations_png/*")

    # 每张图片生成子图的个数
    image_each = image_num // len(images_name)
    image_path, label_path = [], []
    for i in tqdm(range(len(images_name))):
        count = 0
        image = cv2.imread("./dataset/origin/images_png/" + images_name[i])
        label = cv2.imread("./dataset/origin/annotations_png/" + images_name[i], cv2.IMREAD_GRAYSCALE)
